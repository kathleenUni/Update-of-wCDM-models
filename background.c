 ...
 */
/*WHAT I ADDED
CI FUNCTION WHICH IS THE INTEGRAL OF A SINC FUNCTION BECAUSE IT ISNT INBUILT IN CLASS*/
...
int background_w_fld(
  ...
  case BA:
    *w_fld = pba->w0_fld + pba->wa_fld * ((1. - a)/ (2.* pow(a,2.) - 2.*a + 1.));
    break;
  
  case OSCILL:
    *w_fld = pba->w0_fld + pba->wa_fld*(a*sin(1./a) -sin(1.));
    break;
  }
  ...
  //WHAT I ADDED IN
  case JBP:
    *dw_over_da_fld = pba->wa_fld - 2.*pba->wa_fld;
    break;
  ...
  case BA:
    *dw_over_da_fld = pba->wa_fld * ((2.*pow(a,2.) -4.*a +1.)/(pow((2.*pow(a,2.) - 2.*a +1.),2.)));
    break;
  
  case OSCILL:
    *dw_over_da_fld = pba->wa_fld * sin(1./a) - (pba->wa_fld * cos(1./a)) / a;
    break;
  }
  ...
  //WHAT I ADDED IN
  case JBP:
    *integral_fld = 3.*((1.+pba->w0_fld)*log(1./a) + pba->wa_fld*(1. - a) + pba->wa_fld*(pow(a,2.)-1.)/2.);
    //printf("RUNNING JBP");
    break;
      
  case GE:
    if (pba->wa_fld == 0){
      *integral_fld = - 3. *(1. + pba->w0_fld)*log(a);
    } else{
      *integral_fld = (3. / (2.*pba->wa_fld)) * (-pow(1.+ pba->w0_fld,2.) + pow(1. + pba->w0_fld + log(1./a)*pba->wa_fld,2.));
    }
    //*integral_fld =  (3. / 2.) * log(1./a)*(pba->wa_fld*log(a) - 2.*pba->w0_fld - 2.);
    //printf("RUNNING GE");
    break;
  
  case BA:
    *integral_fld = (3./2.)*pba->wa_fld * log(2.*pow(a,2.) - 2.*a + 1.) - 3.*(pba->wa_fld + pba->w0_fld +1.)*log(a);
    //printf("RUNNING BA");
    break;
  
  case OSCILL:
    *integral_fld = (3.) *pba->wa_fld *sin(1.) - (3.)*pba->wa_fld *ci1 + (3.)*(pba->wa_fld *sin(1.) - pba->w0_fld - 1.)* log(a) - 3.*pba->wa_fld*(sin(1./a)*a -ci1a);
    break;
  }
